...
bac_feature1 <- read_excel(pathJoin(save_path, 'association_BF1_bac_img.xlsx'), sheet = 1)
bac_feature1 <- bac_feature1[bac_feature1$Pvalue < 0.05, ]
bac_feature1 <- unique(bac_feature1$Feature)
bac_feature2 <- read_excel(pathJoin(save_path, 'association_BF2_bac_img.xlsx'), sheet = 1)
bac_feature2 <- bac_feature2[bac_feature2$Pvalue < 0.05, ]
bac_feature2 <- unique(bac_feature2$Feature)
...
# blo_feature1 <- read_excel(pathJoin(save_path_blood, 'association_BF1_fecal_img.xlsx'), sheet = 1)
# blo_feature1 <- blo_feature1[blo_feature1$Pvalue < 0.05, ]
# blo_feature1 <- unique(blo_feature1$Feature)
# blo_feature2 <- read_excel(pathJoin(save_path_blood, 'association_BF2_fecal_img.xlsx'), sheet = 1)
# blo_feature2 <- blo_feature2[blo_feature2$Pvalue < 0.05, ]
# blo_feature2 <- unique(blo_feature2$Feature)

# setdiff(blo_feature1, colnames(data_blo))
# intersect(blo_feature1, colnames(data_blo))
blo_feature1 <- colnames(data_blo)[2:ncol(data_blo)]
blo_feature2 <- colnames(data_blo)[2:ncol(data_blo)]
...
association_df <- foreach (i=1:length(blo_feature1),.combine = rbind) %do%  {
  tmp.img_name <- blo_feature1[i]
  img_df <- foreach(j=1:length(bac_feature1), .combine = rbind) %do%  {
    tmp.fea_name <- bac_feature1[j]
    tmp.data <- merge(data_blo[,c(c("ID", tmp.img_name))],
                   data_fea_sub[,c("ID", tmp.fea_name),drop=F],by="ID",all=F)
    tmp.data <- merge(data_cov[,c(c("ID"), COV_fea)], tmp.data, by="ID",all=F)
...
write.xlsx(association_df, file = pathJoin(save_path, 'association_BF1_bac_fecal.xlsx'), rowNames=TRUE)
...
association_df <- foreach (i=1:length(blo_feature2),.combine = rbind) %do%  {
  tmp.img_name <- blo_feature2[i]
  img_df <- foreach(j=1:length(bac_feature2), .combine = rbind) %do%  {
    tmp.fea_name <- bac_feature2[j]
    tmp.data <- merge(data_blo[,c(c("ID", tmp.img_name))],
                   data_fea_sub[,c("ID", tmp.fea_name),drop=F],by="ID",all=F)
    tmp.data <- merge(data_cov[,c(c("ID"), COV_fea)], tmp.data, by="ID",all=F)
...
write.xlsx(association_df, file = pathJoin(save_path, 'association_BF2_bac_fecal.xlsx'), rowNames=TRUE)
